...
var globalPage = GlobalPage.page(),
	...
	all.forEach(function (element) {
		...
		if (element.getAttribute('value') && element.value.length) element.value = _(i18n, fill);
		if (element.getAttribute('placeholder') && element.placeholder.length) element.placeholder = _(i18n, fill);
		if (element.getAttribute('label')) element.label = _(i18n, fill);
		if (element.getAttribute('title')) element.title = _(i18n, fill);
		if (!element.getAttribute('title') && ~['LABEL', 'SPAN', 'DIV', 'P', 'H1', 'LI', 'A', 'H3'].indexOf(element.nodeName) && i18n) element.innerHTML = _(i18n, fill);
	});
